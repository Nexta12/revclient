...
async function getCustomers() {
  try {
    ...
    let mailOptions = {
      ...
      to: [],
      bcc: userEmail,
      subject: "Balance Status Reminder ",
      html: `<h3> Dear Esteemed Customer, </h3> <br>
              <p> This is to warmly remind you of your outstanding balance with us at Revolution Plus, Please kindly pay up to avoid being out of contract with us. </p>
              <br>
              <br>
              <br>
    ...
    if (day == 30)
      filteredDebtors.forEach((debtor) => {
        sendSms(
          ...
          `Dear ${
            debtor.name
          }, this is to remind you of your outstanding balance of NGN ${debtor.properties.grandDebt.toLocaleString()} on ${
            ...
            debtor.username
          } , password: revolutionpluspassword to view more. please change your password after first login`
        );
      });
  } catch (error) {
    console.log(error);
  ...
  // const hashedPassword = await bcrypt.hash("revolutionpluspassword", salt);

  // clients.forEach((client) => {
  //   bcrypt
  //     .compare("revolutionpluspassword", client.password)
  //     .then((pass) => {
  //       //
  //       if (!pass && client.password == 'revplusnew') {
  //         client.updateOne(
  //           {
  //             $set: {
